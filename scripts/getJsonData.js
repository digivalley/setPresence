...
function getJsonData(i){
  return new Promise(resolve => {
	...
	.then(d=> {console.log(JSON.stringify(d) ); return d})/* リターンで戻さないとこの後の処理が空っぽのオブジェクトを扱うことになる */
	.then(data=> resolve(setCnts(data, i)))/**/;
...
function setIndexHtml(v){if(v.imageName){v.imgPath='<img src="./images/'+v.imageName+'" alt="">';}else{v.imgPath=''}if(v.body){v.body='<em>'+v.body+'</em>';}else{v.body=''}let auto,container,containerEnd;if(v.autoPrv){auto=v.autoPrv;}if(v.wrapperType){container='<span class="multiLink">';containerEnd='</span>';}else{container='<span rel="noreferrer" target="_blank" class="openSubWin" href="-'+v.url+'" data-auto="'+auto+'">';containerEnd='</span>';}let html=container+'<dl class="'+v.className+'"><!-- cnt-'+v.k+'+++ --><dt><span class="wrapper">'+v.dateBox+v.imgPath+'<em class="'+v.classHvDate+'">'+v.categoriesName+'</em></span></dt><dd><b>'+v.subheadline+'</b>'+v.body+v.buttonPath+'</dd></dl>'+containerEnd;return html;}
