 $(function(){
	var i ={
		...
		closeText: '最初に戻る',
		moreDspText: 'もっと見る',
		_b: '#lower article.wellnessletter',
		get b(){return this._b},
		get wl(){ return  $(this._b + ' .wrapper div');},
		_clickN: 0,
		get clickN(){return this._clickN},
		set clickN(v){this._clickN= v},
		selfID:  'moreDsp',
		_wlLength: 0,
		//get wlLength(){ return this._wlLength},
		get wlLength(){ return $(this._b + ' .wrapper div').length},
		/* wellnessLetter */
		button: '#nav-collapse',
		...
		fixed: '.fixedWrapper',
		borderLine: 'footer',
		linkBtn: '#system.banner a, #project.banner a, #inaugural a, #vpd a, #spNaviAiside a.outside',
		wrapper:"#scrollNavi",
		brW: 'pc',
	...
	async function asyncCall() {
	  const x = await getJsonData(i);
	  console.log( $('#content').height() ); 
	}
	
	asyncCall();
	eTracking();
	$(i.wrapper).removeClass("scrolldown");
	...
	setPresenceWL(i);
	/* selectList start +++++++++++++++++++++ 
	i.timer4 = setInterval(
		function(){
			i.targetH= parseInt($(i.b +' .wrapper' ).height());
			if( i.targetH >60){
				setPresenceWL(i);
				clearInterval( i.timer4 );
			}else{
				if(i.lgcH == i.targetH){clearInterval( i.timer4 );}
			}
			i.lgcH = i.targetH || 0;
...
function scrollEv(i){
	if(i.brW== 'pc'){
	let minMainH=  parseInt($(i.fixed).height());
	$(i.mainBlock).height(minMainH);
	}
	let fixedLine= $(i.fixed).offset().top - $(i.headerR).height();
	let bottomLine = $(i.borderLine).offset().top -  $(i.borderLine).height();
	...
	$(window).on("scroll", function() {
		...
		fixObj(i.wnH, fixedLine, i.fixed);
		fixObj(i.wnH,i.tgtH2,i.fixed2);/* header menu */
		/* サイドバーのFIX処理 */
		if( scrollBtm < i.wnH ){
